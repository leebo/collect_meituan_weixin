...
WeixinRailsMiddleware::WeixinController.class_eval do
  ...
  private
    ...
    def response_location_message(options={})
      ...
      raidus = 200
      stringlist = ""
      around = get_around(lat, lon, raidus)
      weather = weather(label)
      aroundlist = Merchant.where("lat":{"$gte": around["minLat"], "$lte": around["maxLat"]}, "lng": {"$gt": around["minLng"], "$lt": around["maxLng"]})
      aroundcount = aroundlist.count
      # 最多显示三条
      if aroundcount >= 3
        childlist = Array.new(3)
        index = 0
        childlist.each do |c|
          childlist[index] = "地址:" + aroundlist[index]["address"] + "店铺名:" + aroundlist[index]["name"] + "\n"
          stringlist = stringlist + childlist[index]
          index = index + 1
        end
      else
        childlist = Array.new(aroundcount)
        index = 0
        childlist.each do |c|
          childlist[index] = "地址:" + aroundlist[index]["address"] + "店铺名:" + aroundlist[index]["name"] + "\n"
          stringlist = stringlist + childlist[index]
      ...
      reply_text_message("您的位置为:#{@lx},#{@ly},#{@scale},#{label}\n您所在位置的天气为：\n#{weather}\n您所在位置有#{aroundcount}家美食店\n#{stringlist}")
    end
    # 获得当前位置周围半径最大最小经纬度
    # lat：经度 lon：纬度 raidus：半径
